...
function deleteNotice(daySeq) {
	if(confirm("삭제하시겠습니까?")) {
		...
		$.ajax({
			type:"POST",
			url:"/admin/contents/day_delete_do.go",
			dataType:"json",
			data:param,
				...
				<c:forEach var="it" items="${list}">
					<li class="ui-state-default" style="border: 0 !impotant;" >
						...
						<span class="ui-icon ui-icon-arrowthick-2-n-s"></span>
							...
							<input type="checkbox" name="isLast" value="${it.daySeq}" style="margin-left:3px;">
							...
							<button type="button" class="btn-red" onclick="deleteNotice(${it.daySeq});" >삭제</button>
						</form>
					</li>	
				</c:forEach>
			</c:when>
