...
function deleteNotice(eperSeq) {
	if(confirm("삭제하시겠습니까?")) {
		...
		$.ajax({
			type:"POST",
			url:"/admin/period_manage/no_input_delete_do.go",
			dataType:"json",
			data:param,
				...
				<c:forEach var="it" items="${list}">
					<li class="ui-state-default" style="border: 0 !impotant;" >
						...
						<span class="ui-icon ui-icon-arrowthick-2-n-s"></span>
							<input class="sort"  type="hidden" name="eperSeq" value="${it.eperSeq}">
							<input type="hidden" name="sort" value="${it.sort}">
							<input type="checkbox" name="isLast" class="isLast" value="${it.eperSeq}" style="margin-left:-10px;" ${it.isLast == 1 ? 'checked=\"checked\"' : ''}>
							<span style="display:inline-block; width:15px;  text-align:center">${it.eperSeq}</span>
							...
							<span style="display:inline-block; width:70px; text-align:center; margin-left:4px; margin-right:15px;">
								<c:choose>
									...
									<c:otherwise>
										<input type="hidden" class="ansType2" name="ansType2" value="0" />
									</c:otherwise>
								</c:choose>
							</span>
							...
							<button type="button" class="btn-red" onclick="deleteNotice(${it.eperSeq});" >삭제</button>
						</form>
					</li>	
				</c:forEach>
			</c:when>
