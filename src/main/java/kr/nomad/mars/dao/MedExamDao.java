...
public class MedExamDao {
	...
	public void updateMedExamTable(MedExam medExam) { 
		String query = "" + 
				"UPDATE T_NF_MED_EXAM SET " +
				"	comment = ?, " +
				"	pseq = ?, " +
	...
	// 체크값 변경
	public void updateMedExamIsLast(int is_last ,int medSeq) { 
		
		String query = "" + 
				"UPDATE T_NF_MED_EXAM SET " +
				"	is_last = ? " +
				" WHERE med_seq = ? ";
		this.jdbcTemplate.update(query, new Object[] {is_last,medSeq});
	}
	...
	public List<MedExam> getMedExamList() {
		...
		String query = ""
				...
				+ "	SELECT "
				+ "		* "
				+ "	FROM T_NF_MED_EXAM WHERE kind = 1 order by sort ";
		result = (List<MedExam>)this.jdbcTemplate.query(query, this.medexamMapper);
