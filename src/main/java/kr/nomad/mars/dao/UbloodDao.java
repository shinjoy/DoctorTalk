...
public class UbloodDao {
	...
	public int getUserBloodavg(String id,String now,String before) { 
			String query = "" + 
					"select avg(blood_num) from T_NF_USER_BLOOD where user_id = ? and DATE_FORMAT(reg_date, '%Y-%m-%d') between  ? and  ? "; 
	...
	public List<UserBlood> getUserBlood(String userId) { 
		String query = "" + 
				...
				"WHERE user_id = ? ORDER BY BLO_SEQ desc";
	...
	public UserBlood getUserBlood(String userId, String date, int kind) { 
		String query = ""
				+ "	SELECT * "
				+ "	FROM T_NF_USER_BLOOD "
				+ "	WHERE user_id = ? AND DATE_FORMAT(reg_date, '%Y-%m-%d') = ? AND blood_kind = ? ";
		try {
	...
	public int getCount(String userId, String date) {
	    String query = " SELECT COUNT(*) FROM T_NF_USER_BLOOD WHERE user_id =  ? AND DATE_FORMAT(reg_date, '%Y-%m-%d') = ?  ";
	    return this.jdbcTemplate.queryForInt(query, new Object[] { userId, date });
	}

	/** 유저의 전체 혈당 데이터 갯수 **/
	public int getCount(String userId, String date, int kind) {
	    String query = " SELECT COUNT(*) FROM T_NF_USER_BLOOD WHERE user_id =  ? AND DATE_FORMAT(reg_date, '%Y-%m-%d') = ? AND blood_kind = ?  ";
	...
	public List<UserBlood> getUserBlood(String id,String now,String before) { 
			String query = "" + 
					"select * from T_NF_USER_BLOOD where user_id = ? and DATE_FORMAT(reg_date, '%Y-%m-%d') between  ? and  ? ORDER BY reg_date "; 
	...
	public List<UserBlood> getUserBlood(String id,String now,String before, int bloodKind) { 
		//String query = "select * from T_NF_USER_BLOOD where user_id = ? and blood_kind = ? and DATE_FORMAT(reg_date, '%Y-%m-%d') between  ? and  ? ORDER BY reg_date "; 
		String query = ""
				+ "	select 0 as blo_seq, user_id, blood_time, blood_kind, avg(blood_num) as blood_num, DATE_FORMAT(reg_date, '%Y-%m-%d') as reg_date, 0 as equip "
				+ "	from T_NF_USER_BLOOD "
				+ "		where user_id = ? and blood_kind = ? and DATE_FORMAT(reg_date, '%Y-%m-%d') between '"+before+"' and '"+now+"' "
				+ "	group by user_id, blood_time, blood_kind, DATE_FORMAT(reg_date, '%Y-%m-%d') "
				+ "	ORDER BY reg_date "; 
	...
	public int getUserBloodcnt(String id,String now,String before) { 
		String query = ""  
				...
				+ " from T_NF_USER_BLOOD  where user_id = ? and DATE_FORMAT(reg_date, '%Y-%m-%d') between ? and  ? ";
		 ; 
		return this.jdbcTemplate.queryForInt(query, new Object[] { id,before,now });
	}	
	
	//장비측정 불포함
	public int getUserweekBloodcnt(String id,String now,String before) { 
		String query = ""  
				+ " select count(distinct(DATE_FORMAT(reg_date, '%Y-%m-%d'))) "
				+ " from T_NF_USER_BLOOD  where user_id = ? and equip = 0 and "
				+ " DATE_FORMAT(reg_date, '%Y-%m-%d') between ? and  ? ";
		 ; 
		return this.jdbcTemplate.queryForInt(query, new Object[] { id,before,now });
	}
	...
	public int getmonthCount(String userId, String date,String after) {
	    String query = " SELECT count(distinct(DATE_FORMAT(reg_date, '%Y-%m-%d'))) FROM T_NF_USER_BLOOD WHERE user_id = ? "
	...
	public int getUserBloodavg(String id,String now,String before,int kind) { 
		String query = "" + 
				"select avg(blood_num) from T_NF_USER_BLOOD where user_id = ? and blood_kind= ? and DATE_FORMAT(reg_date, '%Y-%m-%d') between ? and ? "; 
	...
	public int getUserBloodavg(String id,String ym,int kind) { 
		String query = "" + 
				"select avg(blood_num) from T_NF_USER_BLOOD where user_id = ? and blood_kind= ? and DATE_FORMAT(reg_date, '%Y-%m') = ? "; 
				//"select avg(blood_num) from T_NF_USER_BLOOD where user_id = ? and blood_kind= ? and reg_date between ? +' 00:00:00' and ? +' 23:59:00'"; 
		return this.jdbcTemplate.queryForInt(query, new Object[] { id,kind,ym });
	}
	...
	public int getUserBloodcnt(String id,String now,String before,int kind) { 
			String query = ""  
						+ " select count(distinct(DATE_FORMAT(reg_date, '%Y-%m-%d'))) "
						+ " from T_NF_USER_BLOOD  where user_id = ? and blood_kind= ? and DATE_FORMAT(reg_date, '%Y-%m-%d') between  ? and  ? "; 
			return this.jdbcTemplate.queryForInt(query, new Object[] { id,kind,before,now });
	}
	...
	public int getotherBloodavg(String now,String before,int kind) { 
		String query = "" + 
				"select avg(blood_num) from T_NF_USER_BLOOD where blood_kind= ? and DATE_FORMAT(reg_date, '%Y-%m-%d') between  ? and  ?"; 
