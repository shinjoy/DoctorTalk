...
public class UMediDao {
	...
	//해당아이디의 복약리스트
		...
		public List<UserMedi> getUserMediList(String id, int week, String date) {
			...
			String query = ""
					...
					+ "		 "
					+ "		*, "
					+ "		(SELECT count(*) FROM T_NF_USER_MEDICINE_LOG WHERE med_seq = m.medi_seq AND DATE_FORMAT(SYSDATE(), '%Y-%m-%d') = ?) as eat_log "
	...
	public List<UserMedi> getUserMediAdmin(String userId) { 
		String query = "" + 
				"SELECT * " +
				"FROM T_NF_USER_MEDICINE " +
				"WHERE user_id = ? ORDER BY medi_seq desc";
