...
<script type="text/javascript">
	...
	function searchList(listForm,page) {
		var param = {
			page		:	page,
			kcase : listForm.kcase.value
		};
		
		$.ajax({
			type:"POST",
			url:"/admin/period_manage/no_input_list.go",
			dataType:"html",
			data:param,
...
function addListForm(frm) {
		...
		$.ajax({
			type:"POST",
			url:"/admin/period_manage/no_input_edit_do.go",
			dataType:"json",
			data:param,
	...
	function ListEditForm(frm) {
		...
		var param = {
			...
			move  : frm.goseq2.value
		};
		
		$.ajax({
			type:"POST",
			url:"/admin/period_manage/no_input_edit_table_do.go",
			dataType:"json",
			data:param,
			success:function(json){
				alert(json.message);
	...
	function checkQuestion() {
		...
		$.ajax({
			type:"POST",
			url:"/admin/no_input/no_input_checkQuest_do.go",
			dataType:"json",
			data:param,
			success:function(json){
				alert(json.message);
...
*/	
		...
		$.ajax({
			type:"POST",
			url:"/admin/no_input/sort_edit_do.go",
			dataType:"json",
			data:param,
			success:function(json){
				alert(json.message);
						...
						<div class="contents-top">
							<div class="top-tools">
								<div class="search-tool">
									<select name="kcase" >	
										<option value="0">=케이스 검색=</option>
										<c:forEach items="${caseList}" var="it">
											 <option   value="${it.kcase}" ${it.kcase == group ? 'selected=\"selected\"' : '' }>${it.kcase}</option>
										</c:forEach>
									</select>	
<%-- 								<input type="text" name="keyword" class="itext" value="${keyword}" placeholder="제목/내용 검색" > --%>
						...
						<div style="margin-top:20px;">
							...
							<form method="post" name="addForm" id="addForm" >
								<input type="hidden" name="eperSeq"  value="0">
...
<!-- 										<option value="1">안내문</option> -->
								...
								<div class="guide" id="guide" style="display:block; margin-bottom:10px;" >
									...
									<select name="ansType" class="ansType" style="margin-left: 10px;">
									 ...
									 <input type="text" class="itext" placeholder="케이스" name="kcase" style="width:50px;">
								 ...
								 <div class="answer" id="answer" style="display:none; margin-left:10px; margin-bottom:20px;" >
									...
									<input type="text" class="itext" placeholder="케이스" name="k" style="width:30px;">
