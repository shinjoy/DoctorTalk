...
<%-- 					<button type="button" class="btn-blue" onclick="document.location.href='/admin/user/user_edit.go?userId=${user.userId}';">수정</button> --%>
				...
				<table class="register" style="margin-top:10px;">
						...
						<tbody>
							<tr class="center">
								<td>${userCol.regDate.substring(0,10)}</td>
								<td>${userCol.col}</td>
								<td>${userCol.hdl}</td>
								<td>${userCol.tg}</td>
								<td>${userCol.ldl}</td>
					...
					<br>
					<iframe name="graph" src="/m/grape_do.go?userId=${user.userId}&diseaseId=col&speriod=${before}&eperiod=${today}" style="border:0; width:990px; height:280px;"></iframe>
