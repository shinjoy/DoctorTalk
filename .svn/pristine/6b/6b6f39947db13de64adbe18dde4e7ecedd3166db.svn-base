...
<script type="text/javascript">
	...
	function submitForm(frm) {
		...
		var param = {
			...
			hbaNum	: frm.hbaNum.value
		};
		
		$.ajax({
			type:"POST",
			url:"/admin/user/user_hemo.go",
			dataType:"json",
			data:param,
		...
		<section id="contents">
			...
			<div class="contents-block">
				...
				<div class="tab-bar">
					...
					<button type="button" class="tab active" onclick="document.location.href='/pc/hemoglobin_view.go?userId=${user.userId}';"><span>당화혈색소</span></button>
				...
				<div class="tbl-list">
					<table class="register" style="margin-top:10px;">
						...
						<tbody>
							<tr>
								<td style="text-align:left; padding-left:10px;">${userhb.hbaNum}</td>
							</tr>
						</tbody>
					</table>
					
					
					<button class="btn" onclick="windowOpen('/pc/grape.go?userId=${user.userId}&diseaseId=hba','Recover',900,480,'yes','no');" style="margin-top:10px;">그래프 보기</button>
					<form method="post" name="searForm" onsubmit="return searchList(this); return false;">
						<input type="hidden" name="seaId" value="${user.userId}">
							<table class="usertext" style="margin-top:30px; width:100%;">
								...
								<tbody>
								<c:choose>
									<c:when test="${Userhblist.size() > 0}">
										<c:forEach var="it" items="${Userhblist}">
											<tr>
												<td style="text-align:left; padding-left:10px;">${it.hbaNum}</td>
											</tr>
										</c:forEach>
									</c:when>
									<c:otherwise>
				...
				<div>
					...
					<form method="post" name="firmForm" onsubmit="return submitForm(this); return false;">
						...
						<table class="usertext" style="margin-top:30px; width:100%;">
							...
							<tbody>
								<tr>
									<td> <input type="text" class="itext"  name="hbaNum"></td>
