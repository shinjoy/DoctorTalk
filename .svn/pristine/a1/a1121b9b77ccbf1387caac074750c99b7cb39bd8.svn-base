				...
				<c:forEach var="it" items="${list}">
					<li class="ui-state-default" style="border: 0 !impotant;" >
						...
						<span class="ui-icon ui-icon-arrowthick-2-n-s"></span>
							<input class="sort"  type="hidden" name="medSeq" value="${it.medSeq}">
							<input type="hidden" name="sort" value="${it.sort}">
							<input type="checkbox" name="isLast" class="isLast" value="${it.medSeq}" style="margin-left:-10px;" ${it.isLast == 1 ? 'checked=\"checked\"' : ''}>
							<span style="display:inline-block; width:15px; text-align:center">${it.medSeq}</span>
							<span style="display:inline-block; width:75px; text-align:center">${it.sort}</span>
							<span style="display:inline-block; width:35px; text-align:center;">${it.askText}</span>
							<span style="display:inline-block; width:70px; text-align:center; margin-left:7px; margin-right:15px;">
								<c:choose>
