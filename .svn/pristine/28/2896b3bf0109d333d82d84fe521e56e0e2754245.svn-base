...
<!-- 		<li class="subitem" style="width:70px;">구분</li> -->
		<li class="subitem" style="width:100px;">이미지타입</li>
		<li class="subitem" style="width:760px;">내용</li>
...
<%-- 							<input type="checkbox" name="isLast" value="${it.commentSeq}" style="margin-left:3px;"> --%>
							<span style="display:inline-block; width:5px;  text-align:center; margin-right:15px; margin-left:-5px;">${it.commentSeq}</span>
							<span style="display:inline-block; width:100px; text-align:center">
								<select class="imgType" name="imgType" style="width:90px;">
									<option value="red" ${it.imgType == 'red' ? 'selected=\"selected\"' : ''}>위험해요</option>
									<option value="yellow" ${it.imgType == 'yellow' ? 'selected=\"selected\"' : ''}>분발해요</option>
									<option value="green" ${it.imgType == 'green' ? 'selected=\"selected\"' : ''}>좋아요</option>
<%-- 									<option value="" ${it.ansType == 4 ? 'selected=\"selected\"' : ''}>다중선택</option> --%>
								</select>							
							</span> 
...
<%-- 							<span style="display:inline-block; width:70px; text-align:center; margin-right:10px;">${it.valueText}</span> --%>
							<input type="text" name="comment" style="width:750px; margin-left:3px; margin-right:5px;" class="itext" value="${it.commnet}">
