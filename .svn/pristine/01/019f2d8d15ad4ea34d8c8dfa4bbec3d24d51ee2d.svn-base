...
<script type="text/javascript">
	...
	function searchList(searForm,page) {
		...
		document.location.href = "/pc/week_view.go?userId="+searForm.seaId.value+"&page="+page;
		...
		<section id="contents">
			...
			<div class="contents-block">
				...
				<div class="tab-bar">
					...
					<button type="button" class="tab active" onclick="document.location.href='/pc/week_view.go?userId=${user.userId}';"><span>관리주기</span></button>
				...
				<table class="register" style="margin-top:10px;">
						...
						<tbody>
							<tr>
								<td style="text-align:left; padding-left:10px;">${Usercnt.bcntText}</td>
								<td style="text-align:left; padding-left:10px;">${Usercnt.pcntText}</td>
								<td style="text-align:left; padding-left:10px;">${Usercnt.wcntText}</td>
								<td style="text-align:left; padding-left:10px;">${fn:substring(Usercnt.regDate,0,16)}</td>
							</tr>
						</tbody>
					</table>
					...
					<form method="post" name="searForm" onsubmit="return searchList(this); return false;">
						<input type="hidden" name="seaId" value="${user.userId}">
							<table class="usertext" style="margin-top:30px; width:100%;">
								...
								<tbody>
								<c:choose>
									<c:when test="${UserCntlist.size() > 0}">
										<c:forEach var="it" items="${UserCntlist}">
											<tr>
												<td style="text-align:left; padding-left:10px;">${it.bcntText}</td>
												<td style="text-align:left; padding-left:10px;">${it.pcntText}</td>
												<td style="text-align:left; padding-left:10px;">${it.wcntText}</td>
					...
					<form method="post" name="firmForm" onsubmit="return submitForm(this); return false;">
						...
						<table class="usertext" style="margin-top:30px; width:100%;">
							...
							<tbody>
								<tr>
									<td> <input type="text" class="itext"  name="bcnt"></td>
									<td> <input type="text" class="itext" name="pcnt"></td>
									<td> <input type="text" class="itext" name="wcnt"></td>
								</tr>
							</tbody>
						</table>
						<div  class="btn-tools" >	
