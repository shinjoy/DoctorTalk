...
function deleteNotice(cvSeq) {
	if(confirm("삭제하시겠습니까?")) {
		...
		$.ajax({
			type:"POST",
			url:"/admin/doctor_cv_risk/cv_risk_delete_do.go",
			dataType:"json",
			data:param,
...
.datasell {display:table-cell;  width:100%; height:200px; text-align:center; border-top:1px solid #ddd; border-bottom:1px solid #ddd; vertical-align:middle; }
	...
	<ul class="title">
		...
		<li class="subitem" style="width:80px;">관리</li>
	</ul>
				...
				<c:forEach var="it" items="${list}">
					<li class="ui-state-default" >
						...
						<span class="ui-icon ui-icon-arrowthick-2-n-s"></span>
							...
							<input type="checkbox" name="isLast" class="isLast" value="${it.cvSeq}" style="margin-left:-10px;" ${it.isLast == 1 ? 'checked=\"checked\"' : ''}>
							<span style="display:inline-block; width:15px;  text-align:center">${it.cvSeq}</span>
